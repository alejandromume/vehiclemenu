...
if Config.HideCredits then
    ...
    mainMenu = NativeUI.CreateMenu("Vehicle Menu", "~b~Desarrollado por ~w~alejandromume#0884", handleOrientation(Config.MenuPosition), 275)
end
...
local otro7 = NativeUI.CreateItem("Change Color", "Change vehicle color with RGB colors")
--UNCOMMENT ME!
...
mainMenu.OnItemSelect = function(sender, item, index)  
    ...
    elseif item == otro7 then
        car = GetVehiclePedIsIn(GetPlayerPed(-1), false)
        local result1
        local result2
        local result3

        AddTextEntry('FMMC_MPM_NA', "R")
    ...
    SetVehicleCustomPrimaryColour(car, tonumber(result1), tonumber(result2), tonumber(result3))
    ...
    while true do
        ...
        _menuPool:ProcessMenus()
            ...
            if IsControlJustPressed(1, 344) then
                ...
                ESX.TriggerServerCallback("vehiclemenu:fetchUserRank", function(playerRank)
